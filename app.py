...
def cargar_tabla_potreros(archivo) -> pd.DataFrame:
    """
    Lee la hoja BD y reconstruye la tabla larga Potrero -> Ha Prod ->
    Establecimiento, detectando dinámicamente los bloques
    'Cab Prom' / 'Sup Est. (Ha)' / 'EV/Ha' (uno por cada Establecimiento
    que tenga el cliente, sin asumir una cantidad fija).
    """
    raw = pd.read_excel(archivo, sheet_name="BD", header=None, dtype=str)

    fila_header = None
    for i in range(raw.shape[0]):
        if (raw.iloc[i] == "Cab Prom").any():
            fila_header = i
            break
    if fila_header is None:
        raise ValueError(
            "No se encontró en la hoja BD la fila de encabezados de "
            "potreros (columna 'Cab Prom')."
        )

    header = raw.iloc[fila_header]
    datos = raw.iloc[fila_header + 1:]

    registros = []
    for col in range(1, raw.shape[1]):
        if header[col] != "Cab Prom":
            continue
        col_nombre = col - 1
        col_sup_ha = col + 1
        establecimiento = limpiar_texto(header[col_nombre])
        if establecimiento == "":
            continue

        for _, fila in datos.iterrows():
            potrero = limpiar_texto(fila[col_nombre])
            if potrero == "":
                continue
            ha = limpiar_numero(fila[col_sup_ha])
            registros.append(
                {"Potrero": potrero, "Ha Prod": ha, "Establecimiento": establecimiento}
            )
